...
string LinuxParser::Kernel() { 
  ifstream kernel_file(kProcDirectory + kVersionFilename);
  string line, os, v, kernel_version;
  if (kernel_file.is_open()){
    getline(kernel_file, line);
    istringstream linestream(line);
    linestream >> os >> v >> kernel_version;
  }
...
int LinuxParser::RunningProcesses() {
  ...
  if (process_stats_file.is_open()){
    while (getline(process_stats_file, line)){
      istringstream linestream (line);
      if (linestream >> key >> value && key == "procs_running"){
        return value;
      }
    }
  }
...
string LinuxParser::User(int pid) {
  ...
  ifstream passwd_file (kPasswordPath);
  string line, user, user_id;
  char x;
  if (passwd_file.is_open()){
    while (getline(passwd_file,line)){
      std::replace(line.begin(), line.end(), ':', ' ');
      istringstream linestream (line);
      if (linestream >> user >> x >> user_id && user_id == uid){
        return user;
      }
    }
  }
...
long LinuxParser::UpTime(int pid) { 
  ...
  if (process_stat_file.is_open()){
    if (getline(process_stat_file,line)){
      ...
      uptime = system_up_time - start_time;
    }
  }
  return uptime; 
}